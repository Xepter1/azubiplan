...
// Gemeinsame Profil-Ansicht — vom Ausbilder (`/auszubildende/[id]`) und vom Azubi
// selbst (`/mein-profil`) genutzt. `editableClass` blendet die Klassen-Zuordnung
// als Formular ein (Ausbilder) bzw. nur als Wert (Azubi).
export async function AzubiProfilView({
  apprenticeId,
  editableClass,
  context,
}: {
  ...
  const [azubi, departments] = await Promise.all([
    prisma.apprentice.findFirst({
      ...
      select: {
        ...
        grades: {
          select: { wert: true, subject: { select: { name: true } } },
        },
      },
    }),
  ...
  const classes =
    editableClass && azubi.professionId
  ...
  // --- Notenspiegel (echte Durchschnitte: Ø gesamt + je Fach) ---
  const bySubject = new Map<string, number[]>();
  for (const g of azubi.grades) {
    const arr = bySubject.get(g.subject.name) ?? [];
    arr.push(Number(g.wert));
    bySubject.set(g.subject.name, arr);
  }
  const avgOf = (arr: number[]) => arr.reduce((a, b) => a + b, 0) / arr.length;
  const alleNoten = azubi.grades.map((g) => Number(g.wert));
  const oGesamt = alleNoten.length ? fmtNote(avgOf(alleNoten)) : "—";
  const proFach = [...bySubject.entries()]
    .map(([name, arr]) => ({ name, schnitt: fmtNote(avgOf(arr)) }))
  ...
  const eyebrow = context === "ausbilder" ? "Ausbilder" : "Azubi";
  const heading = context === "ausbilder" ? "Azubi-Profil" : "Mein Profil";
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-10">
      ...
      <h1 className="mb-4 text-3xl font-bold tracking-tight">{heading}</h1>
      ...
      <div className="grid gap-6 lg:grid-cols-[20rem_1fr]">
        ...
        <div className="rounded-2xl border bg-card p-6 shadow-sm">
          ...
          <dl className="divide-y text-sm">
            ...
            />
            {!editableClass && (
              <Row label="Klasse" value={azubi.class?.name ?? "—"} />
            )}
          </dl>

          {editableClass && (
            <form action={setApprenticeClass} className="mt-5 border-t pt-4">
              <input type="hidden" name="apprenticeId" value={apprenticeId} />
        ...
        <div className="rounded-2xl border bg-card p-6 shadow-sm">
          ...
          {stations.length === 0 ? (
            <p className="py-6 text-sm text-muted-foreground">
              Für diesen Beruf sind noch keine geeigneten Abteilungen hinterlegt.
            </p>
          ) : (
          ...
          {/* Notenspiegel — echte Durchschnitte */}
          ...
          {alleNoten.length === 0 ? (
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              <div className="rounded-xl border bg-primary/5 px-4 py-3">
                <p className="text-2xl font-bold">{oGesamt}</p>
                <p className="text-xs text-muted-foreground">Ø gesamt</p>
              </div>
              {proFach.map((f) => (
                <div key={f.name} className="rounded-xl border bg-muted/20 px-4 py-3">
                  <p className="text-2xl font-bold">{f.schnitt}</p>
      ...
      <section className="mt-6 rounded-2xl border bg-card p-6 shadow-sm">
        ...
        {sollContents.length === 0 ? (
          <p className="mt-6 text-sm text-muted-foreground">
            Für {azubi.profession?.bezeichnung ?? "diesen Beruf"} sind noch keine
            Lerninhalte hinterlegt.
          </p>
        ) : (
          <div className="mt-6 grid gap-x-8 gap-y-6 md:grid-cols-2">
            <CoverageGroup title="Abgedeckt" state="erledigt" items={abgedeckt} />
            <CoverageGroup title="Fehlt noch" state="offen" items={fehlt} />
            <CoverageGroup title="Eingeplant" state="geplant" items={eingeplant} />
          </div>
        )}
      </section>
    </div>
...
function coverageHint(c: CoverageItem): string {
  if (c.state === "laeuft") return `läuft gerade · ${c.departmentName}`;
